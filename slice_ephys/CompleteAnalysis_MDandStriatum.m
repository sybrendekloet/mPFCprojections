...
RMP_MDM = {'MD3_cell1_ipsi_R_2019_01_28_0002.abf',...
    ...
    %'MD4_3_ipsi_R_2018_11_21_0002.abf', 'MD3_1_ipsi_R_2018_11_21_0002.abf',...
    %'2018_11_16_0020.abf'
    };
...
%     'MD4_3_ipsi_R_2018_11_21_0002.abf',... % removed because of outlier
%     in capacitance
%      '2018_11_16_0020.abf'
    }; % 17 cells, 5 rats
...
%     '2018_11_16_0020.abf','2018_11_16_0002.abf','2018_11_16_0010.abf',...
%     'MD4_1_ipsi_r_2018_11_16_0012.abf',...
%     'MD3_cell1_ipsi_XR_2018_11_29_0002.abf'
    };
Step55Pos_MDM = {'MD3_cell1_ipsi_R_2019_01_28_0005.abf',...
    ...
    'MD4_3_ipsi_R_2018_11_21_0003.abf', 'MD3_1_ipsi_R_2018_11_21_0003.abf',...
%     '2018_11_16_0003.abf','2018_11_16_0011.abf',...
%     'MD4_1_ipsi_r_2018_11_16_0013.abf',...
%     'MD3_cell1_ipsi_XR_2018_11_29_0003.abf'
    };
...
PPR55_AP_MDL = {'MD2_cell2_ipsi_R_2018_11_22_0016.abf','MD3_cell1_ipsi_R_2018_11_22_0021.abf',...
    ...
    '2018_10_19_0004.abf'};
CHIRP70_MDL = {%'MD2_cell2_ipsi_R_2018_11_22_0013.abf','MD3_cell1_ipsi_R_2018_11_22_0024.abf','2018_11_22_0032.abf',...
    %'2018_11_27_0006.abf','2018_11_27_0014.abf','2018_11_27_0024.abf',...
...
CHIRP55_MDL = {%'2018_11_22_0007.abf','MD2_cell2_ipsi_R_2018_11_22_0014.abf','MD3_cell1_ipsi_R_2018_11_22_0023.abf','2018_11_22_0033.abf',...
    %'2018_11_27_0007.abf','2018_11_27_0015.abf','2018_11_27_0034.abf',...
...
figure
subplot(1,3,1)
% R_input.MDL_R_outlier = find(isoutlier(R_input.MDL_R,'median'));
% R_input.MDM_R_outlier = find(isoutlier(R_input.MDM_R,'median'));
% R_input.MDL_R(isoutlier(R_input.MDL_R,'median')) = [];
% R_input.MDM_R(isoutlier(R_input.MDM_R,'median')) = [];
Ri = [R_input.MDL_R R_input.MDM_R];
RiGroups = [zeros(1,size(R_input.MDL_R,2)),ones(1,size(R_input.MDM_R,2))];
%R_input.pNorm = vartestn(Ri',RiGroups','TestType','LeveneAbsolute','Display','off');
%[R_input.p,R_input.h,R_input.stats] = ranksum(R_input.MDL_R,R_input.MDM_R);
...
subplot(1,3,2)
% Tau.MDL_R_outlier = find(isoutlier(Tau.MDL_R,'median'));
% Tau.MDM_R_outlier = find(isoutlier(Tau.MDM_R,'median'));
% Tau.MDL_R(isoutlier(Tau.MDL_R,'median')) = [];
% Tau.MDM_R(isoutlier(Tau.MDM_R,'median')) = [];
Tau2 = [Tau.MDL_R Tau.MDM_R];
TauGroups = [zeros(1,size(Tau.MDL_R,2)),ones(1,size(Tau.MDM_R,2))];
%Tau.pNorm = vartestn(Tau2',TauGroups','TestType','LeveneAbsolute','Display','off');
boxplot(Tau2, TauGroups,'Labels',{'MDL','MDM'})
%[Tau.p,Tau.h,Tau.stats] = ranksum(Tau.MDL_R,Tau.MDM_R);
hold on
...
subplot(1,3,3)
% Capacitance.MDL_R_outlier = find(isoutlier(Capacitance.MDL_R,'median'));
% Capacitance.MDM_R_outlier = find(isoutlier(Capacitance.MDM_R,'median'));
% Capacitance.MDL_R(isoutlier(Capacitance.MDL_R,'median')) = [];
% Capacitance.MDM_R(isoutlier(Capacitance.MDM_R,'median')) = [];
...
%Capacitance.pNorm = vartestn(Capacitance2',CapacitanceGroups','TestType','LeveneAbsolute','Display','off');
boxplot(Capacitance2, CapacitanceGroups,'Labels',{'MDL','MDM'})
%[Capacitance.p,Capacitance.h,Capacitance.stats] = ranksum(Capacitance.MDL_R,Capacitance.MDM_R);
hold on
...
figure
subplot(1,3,1)
% R_input.VS_R_outlier = find(isoutlier(R_input.DS_R,'median'));
% R_input.DS_R_outlier = find(isoutlier(R_input.VS_R,'median'));
% R_input.VS_R(isoutlier(R_input.DS_R,'median')) = [];
% R_input.DS_R(isoutlier(R_input.VS_R,'median')) = [];
Ri = [R_input.DS_R R_input.VS_R];
RiGroups = [zeros(1,size(R_input.DS_R,2)),ones(1,size(R_input.VS_R,2))];
%R_input.pNorm = vartestn(Ri',RiGroups','TestType','LeveneAbsolute','Display','off');
%[R_input.p,R_input.h,R_input.stats] = ranksum(R_input.VS_R,R_input.DS_R);
...
subplot(1,3,2)
% Tau.VS_R_outlier = find(isoutlier(Tau.VS_R,'median'));
% Tau.DS_R_outlier = find(isoutlier(Tau.DS_R,'median'));
% Tau.VS_R(isoutlier(Tau.VS_R,'median')) = [];
% Tau.DS_R(isoutlier(Tau.DS_R,'median')) = [];
Tau2 = [Tau.DS_R Tau.VS_R];
TauGroups = [zeros(1,size(Tau.DS_R,2)),ones(1,size(Tau.VS_R,2))];
%Tau.pNorm = vartestn(Tau2',TauGroups','TestType','LeveneAbsolute','Display','off');
boxplot(Tau2, TauGroups,'Labels',{'DS','VS'})
%[Tau.p,Tau.h,Tau.stats] = ranksum(Tau.VS_R,Tau.DS_R);
hold on
...
subplot(1,3,3)
% Capacitance.VS_R_outlier = find(isoutlier(Capacitance.VS_R,'median'));
% Capacitance.DS_R_outlier = find(isoutlier(Capacitance.DS_R,'median'));
% Capacitance.VS_R(isoutlier(Capacitance.VS_R,'median')) = [];
% Capacitance.DS_R(isoutlier(Capacitance.DS_R,'median')) = [];
...
%[Capacitance.p,Capacitance.h,Capacitance.stats] = ranksum(Capacitance.VS_R,Capacitance.DS_R);
hold on
...
figure
subplot(1,2,1)
% Sag_Ratio20.MDL_R_outlier = find(isoutlier(Sag_Ratio20.MDL_R,'median'));
% Sag_Ratio20.MDM_R_outlier = find(isoutlier(Sag_Ratio20.MDM_R,'median'));
% Sag_Ratio20.MDL_R(isoutlier(Sag_Ratio20.MDL_R,'median')) = [];
% Sag_Ratio20.MDM_R(isoutlier(Sag_Ratio20.MDM_R,'median')) = [];
...
%Sag_Ratio20.pNorm = vartestn(SagRatio',SagRatioGroups','TestType','LeveneAbsolute','Display','off');
boxplot(SagRatio, SagRatioGroups,'Labels',{'MDL','MDM'})
%[Sag_Ratio20.p,Sag_Ratio20.h,Sag_Ratio20.stats] = ranksum(Sag_Ratio20.MDL_R,Sag_Ratio20.MDM_R);
hold on
...
subplot(1,2,2)
% Sag_Ratio20.MDL_R_55_outlier = find(isoutlier(Sag_Ratio20.MDL_R_55,'median'));
% Sag_Ratio20.MDM_R_55_outlier = find(isoutlier(Sag_Ratio20.MDM_R_55,'median'));
% Sag_Ratio20.MDL_R_55(isoutlier(Sag_Ratio20.MDL_R_55,'median')) = [];
% Sag_Ratio20.MDM_R_55(isoutlier(Sag_Ratio20.MDM_R_55,'median')) = [];
...
%Sag_Ratio20.pNorm = vartestn(SagRatio',SagRatioGroups','TestType','LeveneAbsolute','Display','off');
boxplot(SagRatio, SagRatioGroups,'Labels',{'MDL','MDM'})
%[Sag_Ratio20.p_55,Sag_Ratio20.h_55,Sag_Ratio20.stats_55] = ranksum(Sag_Ratio20.MDL_R_55,Sag_Ratio20.MDM_R_55);
hold on
...
figure
% Sag_Ratio20.VS_R_outlier = find(isoutlier(Sag_Ratio20.VS_R,'median'));
% Sag_Ratio20.DS_R_outlier = find(isoutlier(Sag_Ratio20.DS_R,'median'));
% Sag_Ratio20.VS_R(isoutlier(Sag_Ratio20.VS_R,'median')) = [];
% Sag_Ratio20.DS_R(isoutlier(Sag_Ratio20.DS_R,'median')) = [];
...
%[Sag_Ratio20.p,Sag_Ratio20.h,Sag_Ratio20.stats] = ranksum(Sag_Ratio20.VS_R,Sag_Ratio20.DS_R);
hold on
...
% ipsi (64, 65, unknown2)
% MDL_PPR = {'MD3_cell1_ipsi_R_2019_01_24_0001.abf','MD3_cell2_ipsi_R_2019_01_24_0001.abf',...
%     'MD2_cell2_ipsi_R_2019_01_25_0001.abf','MD3_cell1_ipsi_R_2019_01_25_0002.abf','MD3_cell2_ipsi_R_2019_01_25_0004.abf',...
%     'MD3_cell1_ipsi_R_2019_02_07_0003.abf','MD3_cell2_ipsi_R_2019_02_07_0001.abf'};
% %contra (66, 67,68, unknown, 73)
% MDM_PPR = {'MD3_cell1_ipsi_R_2019_01_28_0001.abf',...
%     'MD1_cell1_ipsi_R_2019_01_29_0001.abf','MD1_cell2_ipsi_R_2019_01_29_0001.abf','MD4_cell1_ipsi_R_2019_01_29_0001.abf','MD4_cell2_ipsi_R_2019_01_29_0001.abf',...
%     'MD1_cell1_ipsi_R_2019_01_30_0001.abf','MD1_cell2_ipsi_R_2019_01_30_0001.abf','MD1_cell3_ipsi_R_2019_01_30_0002.abf','MD1_cell4_ipsi_R_2019_01_30_0003.abf','MD2_cell1_ipsi_R_2019_01_30_0001.abf',...
%     'MD3_cell1_ipsi_R_2019_01_31_0001.abf','MD3_cell1_ipsi_R_2019_01_31_0001.abf','MD3_cell3_ipsi_R_2019_01_31_0004.abf','MD3_cell4_ipsi_R_2019_01_31_0003.abf',...
%     'MD3_cell1_ipsi_R_2019_02_05_0002.abf','MD3_cell4_ipsi_R_2019_02_05_0001.abf','MD4_cell1_ipsi_R_2019_02_05_0001.abf'};
...
%% LED APs
% [LED_PPR_APs.MDL_R] = LED_APs(MDL_TABLE.LED_AP_70_MDL);
% [LED_PPR_APs.MDM_R] = LED_APs(MDM_TABLE.LED_AP_70_MDM);
% [LED_PPR_APs.MDL_R_55] = LED_APs(MDL_TABLE.LED_AP_55_MDL);
% [LED_PPR_APs.MDM_R_55] = LED_APs(MDM_TABLE.LED_AP_55_MDM);
