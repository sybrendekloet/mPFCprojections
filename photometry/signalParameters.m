function [test_out,test_stats_pop] = signalParameters(data_in, meta_in)

win1 = ceil(5*1017.3/64);
win2 = ceil(12.5*1017.3/64);
...
%         bar(nanmean(testdata_in{1}), 'facecolor', col_resp(o))
%         hold on
%         errorbar(nanmean(testdata_in{1}),...
%             nanstd(testdata_in{1})/sqrt(size(testdata_in{1},1)), '.')
        
        [P,T,STATS] = kruskalwallis(testdata_in{1},[],'off');
        
        if P<0.05
            [MCT,VALS]=multcompare(STATS, 'CType', 'dunn-sidak','display','off');
            E2 = T{2,5}/((sum(STATS.n)^2-1)/(sum(STATS.n)+1));
            ETA2 = (T{2,5}-numel(STATS.n)+1)/(sum(STATS.n)-numel(STATS.n));
            ETA2_ANOVA = T{2,2}/T{4,2};
            %         ETA2_ANOVA = T{2,5}*(numel(STATS.n)-1)/(sum(STATS.n)-numel(STATS.n));
            title(['***** ' test_fields{idx}])
            ...
            test_stats_pop(o).(test_fields{idx}).ETA2_ANOVA = ETA2_ANOVA;
...
%                 subplot(sub_x, sub_y, idx)
%                 % Remove 0s from data
%                 testdata_in{1} = nan(8,4);
%                 for ou = out_comp
%                     testdata_in{1}(:,ou) = test_out(ou).(test_fields{idx})(:,g);
%                 end
%                 % Take out 2nd col (incorrects)
%                 testdata_in{1} = testdata_in{1}(:,[1 3 4]);
%                 testdata_in{1}(testdata_in{1}==0)=nan;
%                 testdata_in{1}(sum(isnan(testdata_in{1}),2)>0,:)=[];
%                 bar(nanmean(testdata_in{1}), 'facecolor', col_resp(g))
%                 hold on
%                 errorbar(nanmean(testdata_in{1}),...
%                     nanstd(testdata_in{1})/sqrt(size(testdata_in{1},1)), '.')
%                 
%                 [P,T,STATS] = friedman(testdata_in{1},1,'off');
%                 if P<0.05
%                     [MCT,VALS]=multcompare(STATS, 'CType', 'dunn-sidak', 'display','off');
%                     W = T{2.5}/(sum(STATS.n)*(numel(STATS.k)-1));
%                     title(['***** ' test_fields{idx}])
%                     test_stats_out(g).(test_fields{idx}).P = P;
%                     test_stats_out(g).(test_fields{idx}).STATS = STATS;
%                     test_stats_out(g).(test_fields{idx}).MCT = MCT;
%                     test_stats_out(g).(test_fields{idx}).VALS = VALS;
%                     test_stats_out(g).(test_fields{idx}).W = W;
%                 else
%                     title(test_fields{idx})
%                 end
%             end
