function [I_input, N_APs, TimeFirst_AP] = Step_prot_firing_Burst(abfFiles,MDorStr)
...
for fileIx = 1:size(abfFiles,2)
    ...
    for sweep = sweep_pos
        try
            [~,locsFirstPeak] = findpeaks(RAW(upIx:(upIx+samplingFreq/5),V_CH,sweep),'MinPeakHeight',0);
            [pks,locs] = findpeaks(RAW(upIx+locsFirstPeak(1)-5:(upIx+locsFirstPeak(1)+samplingFreq/20),V_CH,sweep),'MinPeakHeight',0);
            N_APs(fileIx,n) = numel(pks)*20;
            TimeFirst_AP(fileIx,n) = locsFirstPeak(1)/samplingFreq;
            %ISI_APs(fileIx,n)
        catch
            N_APs(fileIx,n) = 0;
            TimeFirst_AP(fileIx,n) = nan;
        end
        n = n+1;
    end
...
%     end
    ...
    clearvars -except N_APs I_input fileIx abfFiles V_CH I_CH alternateRecordingDates MDorStr TimeFirst_AP
