function [I_input, N_APs] = Step_prot_firing(abfFiles,MDorStr)
...
for fileIx = 1:size(abfFiles,2)
    ...
    for sweep = sweep_pos
        try
            [pks,locs] = findpeaks(RAW(upIx:(upIx+samplingFreq/2),V_CH,sweep),'MinPeakHeight',0);
            N_APs(fileIx,n) = numel(pks)*2;
            %ISI_APs(fileIx,n)
        catch
            N_APs(fileIx,n) = 0;
        end
